...
def run_pipeline(job_data, resume_text, user_bio):
    user_area = job_data.get('UserArea', {}) or {}
    details = user_area.get('Details', {}) or {}
    job_summary = details.get('JobSummary') or job_data.get('QualificationSummary', '') or f"{job_data.get('PositionTitle', '')} at {job_data.get('OrganizationName', '')}"
